...
namespace FastNetwork
{
    ...
    /// udp 服务器
    /// </summary>
    public sealed class UdpServer : Utils.DisposableBase, IUdpServer
    {
        ...
        private readonly IUdpProtocol _protocol = null;
        private readonly IUdpServerHandler _handler = null;
        #endregion
            ...
            if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)
            {
                ...
                catch (Exception ex)
                {
                   Log.Trace.Error(ex.Message, ex);
                    try { this._handler.OnError(new UdpSession(e.RemoteEndPoint, this), ex); }
                    catch (Exception ex2) {Log.Trace.Error(ex2.Message, ex2); }
                }
                ...
                if (cmdInfo != null)
                {
                    ...
                    {
                        try { this._handler.OnReceived(new UdpSession(e.RemoteEndPoint, this), cmdInfo); }
                        catch (Exception ex)
                        {
                           Log.Trace.Error(ex.Message, ex);
                           try { this._handler.OnError(new UdpSession(e.RemoteEndPoint, this), ex); }
